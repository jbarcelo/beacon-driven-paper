...
If a node has two neighbours transmitting simultaneously, it is assumed that the messages are garbled an lost.
...
These are strong assumptions because, in reality, wireless propagation is much more complicated.
...
\section{Distributed Adjustment of the Schedule Length}
...
\item The schedule is doubled halved if all the neighbors report occupation higher than 75\%.
\item The schedule is halved if all the neighbors report occupation lower than 25\%.
\end{itemize}

\section{Data Included in the Beacons}
...
\section{The Medium Access Control Protocol}
